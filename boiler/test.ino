...
void setup() {
  ...
  pinMode(encoderA, INPUT);digitalWrite(encoderA, HIGH);
  pinMode(encoderB, INPUT);digitalWrite(encoderB, HIGH);
  ...
  if(digitalRead(encoderK) == 0) {
    ...
    case 6:
      ...
      do {
        ...
        Hysteresis = constrain(Hysteresis, 0.1, 3); // крайние значения
      }
      while ((digitalRead(encoderK)==1)|(MenuTimeoutTimer==0));
      if (MenuTimeoutTimer != 0) {
    ...
    case 9:
      ...
      do {
        ...
        AlarmTemp = constrain(AlarmTemp, 15, 85); // крайние значения
      }
